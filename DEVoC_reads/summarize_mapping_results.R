...
for (x in S) {
    ...
    M[idx, x] <- t2[idx, 3] / t2[idx, 5] / tab[x, 'clean_reads'] * 10^6

}


write.table(DF, sep = '\t', quote = F, row.names = F, file = 'DEVoC_DB1_mapping_summary.txt')
write.table(M, sep = '\t', quote = F, file = 'DEVoC_DB1_abundance.txt')
